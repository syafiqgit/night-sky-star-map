...
// ─── Komponen Helper: FilterItem ──────────────────────────────────────────────
function FilterItem({ label, active, onClick }: { label: string, active: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className="flex items-center justify-between w-full p-2 hover:bg-white/5 rounded-lg transition-colors text-left"
    >
      <span className={`text-xs font-mono ${active ? "text-slate-200" : "text-slate-500"}`}>
        {label}
      </span>
...
export default function MapInterface({ lat, lon, time }: Props) {
  ...
  const searchResults = useMemo(() => {
    ...
    // Cari bintang yang memiliki nama dan cocok dengan query, batasi 5 hasil
    return stars
  ...
  const handleStarHover = useCallback((star: HoveredStar | null) => {
    setHoveredStar(star);
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const [starsData, consData] = await Promise.all([
          starsRes.json() as Promise<Star[]>,
          consRes.json() as Promise<Constellation[]>,
        ]);
    ...
    <div className="relative h-full w-full">
      ...
      <div className="absolute top-32 left-6 z-50 pointer-events-auto w-72 flex flex-col gap-2">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search star (e.g. Sirius)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-black/60 backdrop-blur-md border border-white/10 rounded-xl pl-10 pr-10 py-3 text-sm text-slate-200 font-mono focus:outline-none focus:border-blue-500/50 shadow-lg"
          />
          {searchQuery && (
            <button 
              onClick={() => { setSearchQuery(""); setActiveTarget(null); }} 
          ...
          <div className="bg-black/80 backdrop-blur-xl border border-white/10 rounded-xl p-2 flex flex-col gap-1 shadow-2xl animate-in fade-in">
            {searchResults.map((s) => (
              <button
                ...
                onClick={() => {
                  setActiveTarget(s);
                  setSearchQuery(""); // Bersihkan query untuk menutup menu dropdown
                }}
                className="flex items-center justify-between p-3 hover:bg-blue-500/20 rounded-lg transition-colors text-left group"
              >
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-slate-200 group-hover:text-blue-400">{s.name}</span>
                  <span className="text-[10px] font-mono text-slate-500">Mag {s.mag.toFixed(2)}</span>
                </div>
        ...
        {activeTarget && !searchQuery && (
          <div className="bg-green-500/10 border border-green-500/30 backdrop-blur-md rounded-xl p-3 flex items-center justify-between animate-in fade-in shadow-[0_0_15px_rgba(34,197,94,0.15)]">
            <div className="flex items-center gap-3">
              <Target size={20} className="text-green-400 animate-pulse" />
              <div className="flex flex-col">
                <span className="text-[9px] font-bold text-green-400 tracking-[0.2em] uppercase">Target Locked</span>
                <span className="text-sm font-mono text-slate-200 font-bold">{activeTarget.name}</span>
              </div>
            </div>
            <button 
              onClick={() => setActiveTarget(null)} 
              className="p-1 hover:bg-green-500/20 rounded text-green-400/70 hover:text-green-400"
            >
              <X size={16} />
            </button>
          </div>
