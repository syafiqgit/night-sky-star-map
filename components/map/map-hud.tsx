...
import {
  Clock3, Globe2, MapPin, MousePointer2, Layers3,
  Eye, EyeOff, Search, Sparkles, Target, X, Check, Crosshair,
} from "lucide-react";
...
const Panel = memo(function Panel({ children, className = "" }: { children: React.ReactNode; className?: string; }) {
  return (
    <div className={["rounded-2xl border border-white/10 bg-black/50 shadow-[0_8px_32px_rgba(0,0,0,0.4)] backdrop-blur-2xl", className].join(" ")}>
      {children}
    </div>
  );
});
...
const Label = memo(function Label({ children }: { children: React.ReactNode }) {
  return <span className="text-[8px] font-semibold uppercase tracking-[0.24em] text-sky-400/80 sm:text-[9px]">{children}</span>;
});

const Divider = () => <div className="mx-4 h-px bg-white/5" />;

export default function MapHUD({
  lat, lon, time, filters, onToggleFilter, searchQuery, onSearchChange, searchResults, activeTarget, onSelectTarget, onClearTarget, setTime,
}: HUDProps) {
  ...
  const fmt = useMemo(() => {
    if (!mounted) return { time: "--:--:--", date: "--- --, ----", offset: "UTC±--" };
    return { time: TIME_FMT.format(time), date: DATE_FMT.format(time), offset: formatUTCOffset(time) };
  }, [mounted, time]);
  ...
  return (
    <div className="pointer-events-none absolute inset-0 z-40 select-none font-mono text-white">
          ...
          <div className="flex items-center justify-between px-4 py-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="text-[11px] font-semibold text-white">Earth Position</div>
              </div>
            </div>
            <div className="flex items-center gap-1.5 rounded-full border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-1">
              ...
              <span className="text-[8px] font-semibold uppercase tracking-[0.2em] text-emerald-300">Live</span>
            </div>
          </div>
          <Divider />
            ...
            {[
              ...
              <div key={label} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] uppercase tracking-[0.18em] text-slate-500">{label}</span>
                </div>
                <span className="text-[10px] font-medium tabular-nums text-slate-200">{value}</span>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <Panel className="overflow-hidden">
          <div className="relative">
            ...
            {searchQuery && (
              <button
                type="button"
                onClick={() => { onSearchChange(""); onClearTarget(); }}
            ...
            <div className="max-h-64 overflow-y-auto py-1.5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-white/10">
              {searchResults.map((obj) => (
                <button
                  ...
                  className="group flex w-full items-center gap-3 rounded-xl border border-transparent px-3 py-2.5 text-left transition-all hover:border-sky-500/20 hover:bg-sky-500/10 mx-1.5"
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 text-[9px] uppercase tracking-[0.14em] text-slate-600">
                      ...
                      {typeof obj.mag === 'number' ? `mag ${obj.mag.toFixed(2)}` : "mag --"}
                    </div>
                  </div>
                  <Crosshair size={12} className="text-slate-600 group-hover:text-sky-400 transition-colors" />
                </button>
              ))}
            </div>
          </Panel>
      ...
      <div className="pointer-events-auto absolute right-4 top-4 flex flex-col items-end gap-2 sm:right-5 sm:top-5">
        ...
        >
          <Layers3 size={16} className={`transition-transform duration-300 ${showFilters ? "rotate-180" : ""}`} />
        </button>
        ...
        {showFilters && (
          <Panel className="w-52 overflow-hidden">
            ...
            <div className="space-y-1 p-2">
              {(Object.keys(FILTER_META) as Array<keyof MapFilters>).map((key) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      on ? "border-sky-500/25 bg-sky-500/10 text-white" : "border-white/5 bg-white/3 text-slate-500 hover:border-white/10 hover:text-slate-300",
                    ].join(" ")}
                  >
                    <span className="text-[11px] font-medium">{FILTER_META[key].label}</span>
                    <div className={["flex h-5 w-5 items-center justify-center rounded-full transition-all", on ? "bg-sky-500/20 text-sky-300" : "bg-white/5 text-slate-600"].join(" ")}>
        ...
        <div className="flex items-end justify-between gap-3">
          <Panel className="px-4 py-3">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <div className="mt-0.5 text-[8px] text-slate-600">{fmt.date}</div>
              </div>
            </div>
          </Panel>
          ...
          <div className="flex flex-col items-end gap-1.5">
            <Panel className="px-3 py-2">
              <div className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" />
                <span className="text-[8px] font-medium uppercase tracking-[0.2em] text-slate-400">Realtime</span>
              </div>
            </Panel>
            <Panel className="px-3 py-2">
              <div className="flex items-center gap-1.5">
                <MousePointer2 size={10} className="text-slate-500" />
                <span className="text-[8px] text-slate-500">Drag to navigate</span>
              </div>
            </Panel>
          </div>
        </div>
